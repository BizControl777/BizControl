...
import { calcularLucro, custoUnitarioFromCaixa, safeNum } from "../produtos-calc.js";
import {
  buildCartItem,
  cartTotals,
  findProduto,
  ...
  if (confirmBtn) {
    confirmBtn.onclick = async () => {
      ...
      try {
        ...
        console.error(err);
        alert(err?.message || "Erro ao actualizar stock.");
      }
    };
  }
}
...
export function renderCart() {
  ...
  ci.innerHTML = linhas.map((c) => `
    <div class="cart-item">
      <div class="cart-item-main">
        <div><span style="font-size:18px">${c.icon || "<i class='fa-solid fa-box'></i>"}</span> <span style="font-size:13px;font-weight:500">${c.nome || "Produto"}</span></div>
        <div class="cart-item-sub">${fmt(c.preco)} x ${c.qty}</div>
      ...
      <div class="cart-item-side">
        ...
        <span style="color:var(--accent);font-weight:600;min-width:70px;text-align:right">${fmt(c.total)}</span>
      </div>
    </div>
  `).join("");
  ...
  const alerts = [
    lucro < 0 ? "Margem negativa: reduza o desconto antes de finalizar." : "",
...
export function addToCart(id, cart, setCart, produtos) {
  ...
  if (ex) {
    if (ex.qty >= prod.stock) {
      alert(`Stock máximo disponível: ${prod.stock} un.`);
      showCartFeedback(`Stock máximo: ${prod.stock}`);
      return list;
    }
    newCart = list.map((x) => {
      if (normalizeId(x.id) !== pid) return x;
      const qty = x.qty + 1;
      const maxDiscount = (Number(x.preco) || 0) * qty;
      return { ...x, qty, stockMax: prod.stock, desconto: Math.min(Number(x.desconto) || 0, maxDiscount) };
    });
    showCartFeedback(`${prod.nome} — ${ex.qty + 1} un.`);
  } else {
    newCart = [...list, buildCartItem(prod)];
...
export function changeQty(id, delta, cart, setCart, produtos) {
  ...
  const p = findProduto(produtos, pid);
  const stockMax = p ? normalizeProdutoPDV(p).stock : ex.stockMax || 9999;
...
export async function confirmarVenda(cart, setCart, user, vendas, setVendas, produtos, setProdutos, closeModal, showModal) {
  ...
  for (const c of items) {
    ...
    if (!p) {
      alert(`Produto inválido no carrinho (ID ${c.id}).`);
      return;
    }
    const prod = normalizeProdutoPDV(p);
    if (c.qty > prod.stock) {
      alert(`${prod.nome}: stock insuficiente (disponível: ${prod.stock}).`);
      return;
    }
  }
  ...
  if (total <= 0) {
    alert("Total da venda deve ser maior que zero.");
    return;
  }
  if (lucro < 0) {
    alert(`Venda bloqueada: o desconto deixa a margem negativa.\n\nCusto: ${fmt(custoTotal)}\nTotal após descontos: ${fmt(total)}`);
    return;
  }
  ...
  const finalizar = async () => {
    try {
      ...
      if (meta.status_pagamento === "pendente" && meta.cliente_nome === "Cliente balcão") {
        alert("Informe o nome do cliente para venda pendente/crédito.");
        return;
      }
      ...
      const result = await window.api.registarVenda({ itens, total, ...meta });
